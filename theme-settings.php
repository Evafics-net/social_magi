 ...
 */
function social_magi_form_system_theme_settings_alter(&$form, FormStateInterface &$form_state, $form_id = NULL) {
  ...
  // If the default theme is either social_magi or socialsaas then extend
  ...
  if (array_key_exists('socialbase', \Drupal::service('theme.manager')->getActiveTheme()->getBaseThemes())) {
    if ($active_theme == $system_theme_settings) {
      ...
      $form['#submit'][] = 'social_magi_save_email_logo';
      ...
      if (\Drupal::configFactory()->get('system.theme')->get('admin') === 'gin') {
        $form['#submit'][] = 'social_magi_update_gin_color_settings';
      }
    }
...
/**
 * Updates the gin color settings from social_magi it's config.
 *
 ...
 */
function social_magi_update_gin_color_settings(array $form, FormStateInterface $form_state) {
  // Grab the default social_magi colors, these are set if the color settings
  // aren't overridden yet.
  $default_colors = \Drupal::configFactory()->getEditable('social_magi.settings')->getRawData();
  ...
  $social_magi_colors = \Drupal::configFactory()->getEditable('color.theme.social_magi')->getRawData();
  ...
  $brand_primary = !empty($social_magi_colors) ? $social_magi_colors['palette']['brand-primary'] : $default_colors['color_primary'];
  $brand_secondary = !empty($social_magi_colors) ? $social_magi_colors['palette']['brand-secondary'] : $default_colors['color_secondary'];
 ...
 * @throws \Drupal\Core\Entity\EntityStorageException
 */
function social_magi_save_email_logo(array $form, FormStateInterface $form_state) {
